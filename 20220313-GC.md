...
- 吞吐量：单位时间内的处理能力
  - 对比时不能只看吞吐量，还要将 "检查对象类型"  & "mutator的动作" 考虑在内
...
- What：分"标记"和"清除"阶段。前者标记所有活动对象，后者将没有标记的对象（非活动对象）回收
	- 代码实现：
...
- 优点：实现简单（所以和其他算法的组合也简化）、与保守式 GC 算法兼容（规则：对象不能被移动）

- 缺点：

  - 碎片化fragmentation（增加访问时间，可以通过"压缩"及"BiBOP 法"解决）
  - 分配速度慢
...
- 描述：记录"有多少处程序"引用自己的计数器
  ...
  - 其他CG算法中，没有分块时 mutator 会调用 grabage_collect 函数分配空闲的内存空间

  - 但引用计数法中不会，在 mutator 过程中只会通过增减计数器值来进行内存管理，两种情况下计数器的值会发生增减：
...
- 优点：
  ...
  - 最大暂停时间短
  - 无需从根开始沿着指针进行查找
    - 由特性衍生的做法： 在各个计算节点内回收垃圾时使用"GC标记-清除算法"，在考虑节点间引用关系时采用"引用计数法"
...
- 描述：把内存分为两个部分，把 A 空间的活动对象复制到 B 空间，然后倾倒 A 空间
